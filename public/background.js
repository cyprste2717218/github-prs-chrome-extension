...
async function saveLocalRepoDetails(activeNumPRs) {
	...
	activeNumPRs.map((repoDetails) => {
		...
		chrome.storage.local.set({repoName: numActivePRs}).then(() =>
...
async function saveLocalCurrentStep(step) {
	await chrome.storage.local.set({currentStep: step}).then(() =>
	console.log('set the step'))
}
...
export { saveLocalRepoDetails, getLocalRepoDetails, saveLocalCurrentStep, getLocalCurrentStep }